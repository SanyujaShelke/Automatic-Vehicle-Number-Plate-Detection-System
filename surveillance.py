...
def send_msg(frame, frame_no, cropped_frame, license_no, timestamp, score, final_string):
    detected_plates.append(license_no)
    _, encoded_frame = cv2.imencode(".jpg", frame)
    _, encoded_cropped_frame = cv2.imencode(".jpg", cropped_frame)

    (uname, toaddr, _) = lic_db.get(license_no, "mescoe1@outlook.com")

    add_entry(uname, license_no, timestamp)
    print("sending email to : " , license_no)
    try:
        # send_mail(toaddr, [encoded_frame, encoded_cropped_frame], license_no, timestamp)
        pass
    except:
        print("Problem to send an email")
    # print("MSG Sent : ", frame_no, score, license_no, final_string)
    print("MSG Sent : ", license_no)
...
def audio_loop():
    global audio_indx
    while True:
        if audio_indx < len(detected_plates):
            generate_voice(detected_plates[audio_indx])
            audio_indx += 1
        time.sleep(0.1)


def start_video_surveillance(input_path):
    db = get_all_number_plates()
    for (uname, email, mo_no, lic_no) in db:
        lic_db[lic_no] = (uname, email, mo_no)
